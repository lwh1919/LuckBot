...
from luckbot.core.runtime import RuntimeProfile, RuntimeSpec, run_runtime
...
class _RunHookCapture(LuckbotPlugin):
    ...
    async def _before_run(self, inp: object):
        ...
        history = list(inp.conversation_history)
        history.append(HumanMessage(content="prefilled"))
...
async def test_run_runtime_rejects_external_manager_plus_owned_plugins() -> None:
    pm = PluginManager()
    with pytest.raises(ValueError, match="外部 PluginManager 模式下不可再传 plugins 或 providers"):
        await run_runtime(
            RuntimeSpec(
                system_prompt="x",
                max_steps=1,
