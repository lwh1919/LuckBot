"""内置插件自动发现：扫描 luckbot.plugins.builtin 下所有 *_plugin/ 包。
...
内置与外部的唯一区别：内置包已在 Python 包路径中，用
  importlib.import_module("luckbot.plugins.builtin.<name>") 直接导入；
...
def discover_builtin_plugins():
    """扫描内置 *_plugin/ 包，返回自动实例化的插件列表。"""
    from luckbot.core.plugin.base import LuckbotPlugin  # 局部 import 避免循环
