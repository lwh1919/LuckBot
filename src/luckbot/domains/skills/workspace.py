...
class SkillWorkspace:
    ...
    @staticmethod
    def _resolve_cwd(skill_staged: str, cwd: str) -> str | None:
        """仅允许落在当前 skill staging 目录内。"""
        if not cwd:
            return skill_staged
        candidate = os.path.realpath(os.path.join(skill_staged, cwd))
        root = os.path.realpath(skill_staged)
        try:
