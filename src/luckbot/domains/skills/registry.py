...
_Fingerprint = tuple[tuple[str, float, int], ...]
...
class SkillRegistry:
    ...
    def doc_names(self, skill_name: str) -> list[str]:
        ...
        if skill is None:
            return []
        return [doc.name for doc in skill.docs]
    ...
    def load_doc(self, skill_name: str, doc_name: str) -> str | None:
        ...
        if skill is None:
            return None
        doc = _resolve_doc_ref(skill.docs, doc_name)
        if doc is None:
            return None
        try:
    ...
    def _build_scan_paths() -> list[str]:
        paths = [resolve_project_path(path) for path in _DEFAULT_PROJECT_SCAN_PATHS]
        paths.extend(_DEFAULT_USER_SCAN_PATHS)
        extra = os.getenv("LUCKBOT_SKILLS_DIR", "")
        if extra.strip():
            for p in extra.split(","):
                p = p.strip()
                if p:
                    paths.append(resolve_project_path(p))
        deduped: list[str] = []
        seen: set[str] = set()
        for path in paths:
            if path in seen:
    ...
    def _compute_fingerprint(self) -> _Fingerprint:
        ...
        for skill_file in _iter_skill_files(self._scan_paths):
            try:
                for md_file in _iter_skill_markdown_files(skill_file):
                    stat = os.stat(md_file)
                    entries.append((md_file, stat.st_mtime, stat.st_size))
            except OSError:
                pass
...
def _iter_skill_files(scan_paths: list[str]) -> list[str]:
    files: list[str] = []
...
def _iter_skill_markdown_files(skill_file: str) -> list[str]:
    skill_dir = os.path.dirname(skill_file)
    md_files: list[str] = [skill_file]
    for root, dirs, files in os.walk(skill_dir):
        ...
        for fname in sorted(files):
            ...
            if os.path.abspath(full_path) == os.path.abspath(skill_file):
                continue
...
def _scan_docs(skill_dir: str) -> list[SkillDoc]:
    ...
    for root, dirs, files in os.walk(skill_dir):
        ...
        for fname in sorted(files):
            ...
            desc = _extract_doc_description(full_path, rel_path)
            docs.append(SkillDoc(name=rel_path, description=desc, path=full_path))
    return docs
